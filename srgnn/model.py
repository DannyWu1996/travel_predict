...
class Model(object):
    ...
    def forward(self, re_embedding, train=True):
        ...
        # attention weight for hist item respect to  last item
    ...
    def run(self, sess, fetches, tar, item, adj_in, adj_out, alias, mask):
        return sess.run(fetches, feed_dict={self.tar: tar, self.item: item, self.adj_in: adj_in,
                                                 self.adj_out: adj_out, self.alias: alias, self.mask: mask})

    def save(self, sess, file_path, epoch):
        saver = tf.train.Saver()
        saver.save(sess, file_path)
        if epoch==0 and os.path.exists(file_path):
            raise FileExistsError("%s file is already existed, please check first" % file_path)
        print("current best model saved in %s" % file_path)

    def restore(self, sess, file_path):
        if os.path.exists(file_path):
            raise FileExistsError("%s doesn't exist" % file_path)
...
class GGNN(Model):
    def __init__(self, hidden_size=100, out_size=100, batch_size=300, n_node=None,
        ...
        with tf.variable_scope('ggnn_model', reuse=None):
            # self.ggnn() return the node embedding of the current 
    ...
    def ggnn(self):
        #input of the gnn, from global embedding
        ...
        with tf.variable_scope('gru'):
            for i in range(self.step):
                ...
                # concate the fin_state with influx weight sum and outflux weight sum
