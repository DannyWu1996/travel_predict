...
class Seq2SeqAttNN(NN):
    ...
    def build_model(self):
        ...
        sco_mat = tf.matmul(prod,self.embe_dict,transpose_b= True)
            ...
            while bt.has_next():    # batch round.
                ...
                if batch_lenth > self.batch_size:
                    ...
                    for x in range(patch_len):
                        ...
                        # print(batch_data['out_idxes'][i:i+self.batch_size])
                        tmp_out_data = np.reshape(batch_data['out_idxes'][i:i+self.batch_size], [-1])
                        
                        tmp_in_len = batch_data['seq_lens'][i:i+self.batch_size]
                        feed_dict = {
                            self.inputs: tmp_in_data,
                            self.last_inputs: [s[-1] for s in tmp_in_data],
                            self.lab_input: tmp_out_data,
                            self.sequence_length: tmp_in_len
                        }
                        
                        # print(tmp_out_data.shape)
                            # train
                        crt_loss, crt_step, opt, embe_dict, logits = sess.run(
                            [self.loss, self.global_step, self.optimize, self.embe_dict, self.softmax_input],
                            feed_dict=feed_dict
                        )
                        # if x==0:
                        #     print(logits)
                        # if np.sum(np.isnan(crt_loss))>0:
                        #     print("target:{}".format(tmp_out_data[np.isnan(crt_loss)]))
                    ...
                    if remain > 0:
                        ...
                        tmp_out_data = np.reshape(batch_data['out_idxes'][i:], [-1])
                        tmp_in_len = batch_data['seq_lens'][i:i+self.batch_size]
                            # train
                        feed_dict = {
                            self.inputs: tmp_in_data,
        ...
        while bt.has_next():    # batch round.
            ...
            if batch_lenth > self.batch_size:
                ...
                for x in range(patch_len):
                    ...
                    t_r, t_m, ranks = cau_recall_mrr_org(preds, tmp_out_data, cutoff=self.cut_off)
