...
def batch_all(inputs=[[]], labels=[], class_num=0):
    ...
    # j is the  total number of in_idxes
    for j in range(len(inputs[0])):  # get this batch data.
        for i in range(len(inputs)):
            # rins[0] store hist_i, rins[1] stores labels
            hist = copy.deepcopy(inputs[i][j])
            if i==0:
                rins[i].append(hist+[0]*(class_num-len(hist)))
            else:
                rins[i].append(hist)
            crt_len = len(hist)
            rinlens[i].append([crt_len])
